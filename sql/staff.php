...
try {
    $metrics['prestiti'] = (int) $pdo
        ->query("SELECT COUNT(*) FROM biblio_copy WHERE status_cd NOT IN ('', 'in')")
        ->fetchColumn();
} catch (\PDOException $e) { /* non bloccante */ }

try {
    $metrics['in_scadenza'] = (int) $pdo
        ->query("SELECT COUNT(*) FROM biblio_copy WHERE due_back_dt = CURDATE() AND status_cd NOT IN ('', 'in')")
        ->fetchColumn();
} catch (\PDOException $e) { /* non bloccante */ }

try {
    $metrics['utenti_attivi'] = (int) $pdo
        ->query("SELECT COUNT(*) FROM member WHERE suspended = 'N'")
        ->fetchColumn();
} catch (\PDOException $e) { /* non bloccante */ }
        ...
        <section class="staff-card">
            ...
            <ul class="staff-card-list">
                ...
                <li class="staff-card-item staff-card-item--prep">
                    <div class="staff-card-item-body">
                        <span class="staff-link-static">Arricchimento automatico record <span class="staff-feature-pill">in preparazione</span></span>
                        <p class="staff-card-note">Script di bulk enrichment via Google Books API per completare i metadati dei record con ISBN già presenti.</p>
                    </div>
                </li>
                ...
                <li class="staff-card-item staff-card-item--prep">
                    <div class="staff-card-item-body">
                        <span class="staff-link-static">Acquisizione da OPAC SBN <span class="staff-feature-pill">in preparazione</span></span>
                        <p class="staff-card-note">Collegamento a servizi esterni per scaricare record MARC e importarli direttamente nel catalogo.</p>
                    </div>
                </li>
